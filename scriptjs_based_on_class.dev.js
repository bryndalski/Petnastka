...
/*#__PURE__*/
function () {
  function pictureStatistic(status, imgX, imgY, picturesCounter) {
    _classCallCheck(this, pictureStatistic);
  ...
  _createClass(pictureStatistic, [{
    key: "possibilities",
    value: function possibilities(i) {
    ...
    value: function moveMaker(i) {
      ...
      var temporaryId = picturesObjectArray[this.imgPosition + i].imgPosition; // TODO ZAMIENŃ
...
function buttonMaker() {
  var buttonContainer = document.body.querySelector('.buttonContainer');

  for (i = 3; i <= 6; i++) {
    var buttonek = document.createElement('button');
...
function restarter() {
  ...
  var spooksCont = document.createElement('div');
...
function imageSlicer(i) {
  ...
  var intervalCounter = Math.pow(i, i);
  playInterval = setInterval(function () {
    intervalCounter--;
    picturesObjectArray[picturesObjectArray.find(function (e) {
      return e.status == "EMPTY";
    }).imgPosition].whiteBoxMove(i);
...
function clock() {
  var newHour = new Date(Date.now() - firstDate);
